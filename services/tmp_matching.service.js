...
/**
* 将多余男性轮转分配到前面的满编组
* @param {Array} groups - 小组列表
* @param {Array} maleIds - 待分配的男性ID列表
* @param {number} limitGroups - 只在前 limitGroups 个组中轮转
*/
function distributeExtraMalesRoundRobin(groups, maleIds, limitGroups) {
  if (!Array.isArray(groups) || groups.length === 0 || !Array.isArray(maleIds) || maleIds.length === 0) return;
  const n = Math.min(limitGroups, groups.length);
  let idx = 0;
  for (const mid of maleIds) {
      groups[idx].male_ids.push(mid);
      idx = (idx + 1) % n;
  }
}
...
function generateGroups(participants, selections, matchmakerPicks, options) {
  ...
  if (fullGroupsCount > 0) {
      if (remainingFemales.length >= 2) {
          // 新增一个额外小组：包含所有剩余女生 + 最多 targetMalePerGroup 个剩余男生
          const extraGroup = {
          ...
          // 放入全部剩余女生
          for (const fid of remainingFemales) extraGroup.female_ids.push(fid);
          // 放入不超过 targetMalePerGroup 的男生
          const maleToTake = Math.min(remainingMales.length, targetMalePerGroup);
          const takenMales = remainingMales.splice(0, maleToTake);
          for (const mid of takenMales) extraGroup.male_ids.push(mid);
          groups.push(extraGroup);

          // 如仍有剩余男生，回填到前面的满编组（不含刚加的额外组）
          if (remainingMales.length > 0) {
              distributeExtraMalesRoundRobin(groups, remainingMales, fullGroupsCount);
          }
      } else if (allRemainingParticipants.length > 0) {
          // 保持原有逻辑：平均分配到前N个满编组
